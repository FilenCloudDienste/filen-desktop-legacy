...
export const sortTasks = async ({ uploadToRemote, downloadFromRemote, renameInLocal, renameInRemote, moveInLocal, moveInRemote, deleteInLocal, deleteInRemote, location }: { uploadToRemote: any, downloadFromRemote: any, renameInLocal: any, renameInRemote: any, moveInLocal: any, moveInRemote: any, deleteInLocal: any, deleteInRemote: any, location: Location }): Promise<any> => {
    const ignored = []

    const [{ selectiveSyncRemoteIgnore, filenIgnore }, syncMode] = await Promise.all([
        getIgnored(location),
                ...
                maxSyncTasksSemaphore.acquire().then(() => {
                    ...
                    const doTask = (lastErr?: any) => {
                        ...
                        fsRemote.rename(task.type, task).then((done) => {
                            ...
                            ipc.addToApplyDoneTasks(location.uuid, doneTask).then(() => {
                                maxSyncTasksSemaphore.release()
