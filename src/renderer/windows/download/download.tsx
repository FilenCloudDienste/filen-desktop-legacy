	...
	return new Promise((resolve, reject) => {
		fsLocal
			...
			.then(tmpDir => {
				...
				Promise.all([fsLocal.rmPermanent(absolutePath), fsLocal.rmPermanent(fileTmpPath)])
					.then(async () => {
...
const DownloadFolder = memo(
	({
					...
					apiRequest({
						...
						.then(async response => {
							...
							iterateTree(tree[0], () => {
								...
								fsLocal
									.smokeTest(pathModule.normalize(pathModule.join(baseDownloadPath, "..")))
									.then(() => {
										fsLocal
											.rmPermanent(baseDownloadPath)
											.then(() => {
												fsLocal
													.mkdirNormal(baseDownloadPath, {
														recursive: true
													})
													.then(async () => {
														...
														for (const path in obj.folders) {
															try {
																await fsLocal.mkdirNormal(
																	pathModule.normalize(pathModule.join(baseDownloadPath, path)),
																	{
																		recursive: true
																	}
																)
														...
														if (foldersCreated == Object.keys(obj.folders).length) {
																					...
																					.then(() => {
																						downloadFile(
																							pathModule.normalize(
																								pathModule.join(baseDownloadPath, path)
																							),
																							obj.files[path]
																						)
																							.then(() => {
															...
															showToast({
																message: "Could not create needed folders locally",
																status: "error"
															})

															log.error(
																new Error(
																	"Could not create needed folders: " +
																		Object.keys(obj.folders).length +
																		" -> " +
																		foldersCreated
																)
															)
		...
		useEffect(() => {
			ipc.getAppPath("downloads")
				.then(path => setDownloadPath(pathModule.normalize(pathModule.join(path, args.name))))
				.catch(log.error)
										...
										onClick={() => {
											ipc.selectFolder()
												.then(result => {
													...
													Promise.all([fsLocal.smokeTest(localPath)])
														.then(() => {
...
const DownloadFile = memo(
	({
		...
		const startDownloading = () => {
			...
			fsLocal
				.smokeTest(downloadPath)
				.then(() => {
					fsLocal
						.remove(downloadPath)
						.then(() => {
							downloadFile(downloadPath, args.file)
								.then(() => {
									setIsDownloading(false)
								})
								.catch((err: any) => {
