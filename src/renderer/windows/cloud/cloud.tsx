...
const CloudItem = memo(
	({
		...
		darkMode: boolean
		index: number
		navigateToFolder: (uuid: string) => void
	}) => {
		const [icon, setIcon] = useState<string>("")

		useEffect(() => {
			if (item.type == "file") {
				ipc.getFileIconName(item.metadata.name)
					.then(gotIcon => {
						setIcon(gotIcon)
					})
					.catch(log.error)
		...
		>
			...
			>
				...
				>
					...
					/>
					<ModalHeader color={colors(platform, darkMode, "textPrimary")}>{i18n(lang, "createFolder")}</ModalHeader>
					<ModalBody>
						<Input
							type="text"
