...
export const showToast = ({ title = "", description = "", message = "", status = "info", duration = 5000, isClosable = false }) => {
    ...
    toast({
        ...
        duration,
        isClosable,
        render: () => <Toast message={description.length > 0 ? description : message} status={status} />
    })
}

const Toast = memo(({ message, status }) => {
