...
const decryptFolderMetadata = async (str, userMasterKeys, uuid) => {
	...
	let cacheKey = uuid + "_folder_" + str
...
const decryptFileMetadata = async (metadata, userMasterKeys, uuid) => {
	let cacheKey = uuid + "_file_" + metadata
...
const downloadFileChunk = async (file, index, tries, maxTries, isSync, callback) => {
	...
	$.ajax({
		...
		success: (res, textStatus, xhr) => {
			...
			if(xhr.status == 404){
				return setTimeout(() => {
					downloadFileChunk(file, index, 99999999999, maxTries, isSync, callback)
				}, 1000)
			}
...
const getLocalSyncDirContents = async (callback) => {
	...
	readdirp(winOrUnixFilePath(userSyncDir), {
		...
		return callback(null, folders, files, iNodeMapINodesRes, iNodeMapOnlyINodesRes)
	})
}
	...
	apiRequest("/v1/get/dir", {
		...
		if(!reloadAll){
			...
			if(typeof lastRemoteSyncDataHash !== "undefined"){
				if(receivedDataHash == lastRemoteSyncDataHash && typeof lastRemoteSyncFolders !== "undefined" && typeof lastRemoteSyncFiles !== "undefined" && typeof lastRemoteUUIDs !== "undefined" && typeof lastRemoteUUIDsOnlyUUIDs !== "undefined"){
					//console.log("Last remote sync data identical to current one, serving from cache.")

					return callback(null, lastRemoteSyncFolders, lastRemoteSyncFiles, lastRemoteUUIDs, lastRemoteUUIDsOnlyUUIDs) 
				}
			}
			...
			if(self.parent !== "base"){
				if(typeof folders[self.uuid] !== "undefined"){
					...
					if(typeof newPath !== "undefined"){
						...
						remoteUUIDsOnlyUUIDsRes[self.uuid] = {
							path: newPath,
							folder: folders[self.uuid]
						}
					}
		...
		return callback(null, folderPaths, files, remoteUUIDsRes, remoteUUIDsOnlyUUIDsRes)
	})
}
...
const syncTask = async (where, task, taskInfo, userMasterKeys) => {
	...
	for(let i = 0; i < currentSyncTasks.length; i++){
		if(currentSyncTasks[i].indexOf(taskId) !== -1){
			taskIdFound = true
		}
	}
	...
	if(currentRenamingLocalFolders.length > 0){
		for(let i = 0; i < currentRenamingLocalFolders.length; i++){
			if(prop.indexOf(currentRenamingLocalFolders[i]) !== -1){
				isRenamingParentFolderLocal = true
			}
		}
	}

	if(currentMovingLocalFolders.length > 0){
		for(let i = 0; i < currentMovingLocalFolders.length; i++){
			if(prop.indexOf(currentMovingLocalFolders[i]) !== -1){
				isMovingParentFolderLocal = true
			}
		}
	}
...
const doSync = async () => {
	...
	fs.access(winOrUnixFilePath(userSyncDir), async (err) => {
		if(err){
			...
			getRemoteSyncDirContents(folderUUID, (err, remoteFolders, remoteFiles, remoteUUIDsRes, remoteUUIDsOnlyUUIDsRes) => {
				if(err){
					isSyncing = false
					isIndexing = false
					canRemoveFromSyncTasks = true
				...
				getLocalSyncDirContents(async (err, localFolders, localFiles, iNodeMapINodesRes, iNodeMapOnlyINodesRes) => {
					if(err){
						isSyncing = false
						isIndexing = false
						canRemoveFromSyncTasks = true
					...
					currentRenamingLocalFiles = []
					currentRenamingRemoteFiles = []
								...
								for(let prop in iNodeMapINodesRes){
									...
									if(typeof ino == "number"){
										if(typeof iNodeMapOnlyINodes[ino] !== "undefined" && typeof iNodeMapOnlyINodesRes[ino] !== "undefined"){
											if(iNodeMapOnlyINodesRes[ino] !== iNodeMapOnlyINodes[ino]){
												let oldPath = iNodeMapOnlyINodes[ino]
								...
								for(let prop in remoteUUIDsRes){
									...
									if(typeof lastRemoteUUIDsOnlyUUIDs[uuid] !== "undefined" && typeof remoteUUIDsOnlyUUIDsRes[uuid] !== "undefined"){
										if(remoteUUIDsOnlyUUIDsRes[uuid].path !== lastRemoteUUIDsOnlyUUIDs[uuid].path){
											let oldPath = lastRemoteUUIDsOnlyUUIDs[uuid].path
											let newPath = remoteUUIDsOnlyUUIDsRes[uuid].path
									...
									if(fileStat){
										if(typeof localFileModifications[prop] == "undefined"){
											if(fileStat.mtimeMs > 1399784066403){
												localFileModifications[prop] = fileStat.mtimeMs
											}
											else{
												delete localFileModifications[prop]
											}
											...
											if(fileStat.mtimeMs > localFileModifications[prop] && fileStat.mtimeMs > 1399784066403 && localFileModifications[prop] > 1399784066403){
							...
							if(typeof localFiles[prop] == "undefined"){
								...
								if(syncMode == "twoWay"){
									if(typeof lastRemoteSyncFiles[prop] !== "undefined" && typeof localFileExisted[prop] !== "undefined"){
										...
										if(remoteFiles[prop].size > 0 && canDownloadFile(prop)){
...
const indexCleanup = async () => {
	...
	for(let prop in localFileExisted){
		existingFilesFullPaths[prop] = true
	}

	for(let prop in lastRemoteSyncFolders){
		existingFolderUUIDs[lastRemoteSyncFolders[prop].uuid] = true
	}

	for(let prop in lastRemoteSyncFiles){
		existingFileUUIDs[lastRemoteSyncFiles[prop].uuid] = true
	}

	for(let prop in localFileModifications){
		if(typeof existingFilesFullPaths[prop] == "undefined"){
			delete localFileModifications[prop]
		}
	}
	...
	for(let prop in remoteDecryptedCache){
		let ex = prop.split("_")

		if(ex[1] == "file"){
			if(typeof existingFileUUIDs[ex[0]] == "undefined"){
				delete remoteDecryptedCache[prop]
			}
		}
		else if(ex[1] == "folder"){
			if(typeof existingFolderUUIDs[ex[0]] == "undefined"){
				delete remoteDecryptedCache[prop]
			}
		}
		else{
...
const startSyncing = async () => {
	...
	checkIfSyncFolderExistsRemote(async (err, exists, uuid) => {
		...
		try{
			...
			showBigErrorMessage("Could not load local database, please restart the application and try again.")

			return console.log(e)
		}

		if(typeof userEmail == "undefined"){
			return showBigErrorMessage("Could not load local database, please restart the application and try again.")
		}
