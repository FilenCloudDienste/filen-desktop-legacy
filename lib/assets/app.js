	...
	apiRequest(url, {
		...
		if(currentDownloadFolderIsShared){
			try{
		    	...
		    	basePath = new TextDecoder().decode(decrypted)
			}
			catch(e){
				return callback(e)
			}
		...
		for(let i = 0; i < res.data.folders.length; i++){
			...
			if(currentDownloadFolderIsShared){
				try{
			    	...
			    	selfName = new TextDecoder().decode(decrypted)
				}
				catch(e){
					return callback(e)
				}
			...
			for(let prop in files){
				...
				downloadFileToLocal(winOrUnixFilePath(path), files[prop], false, (err) => {
					if(err){
						...
						console.log(path + " done downloading")
					}
				})
			}
		}
	...
	apiRequest("/v1/user/masterKeys", {
		...
		try{
			...
			if(newKeys.length > 16){
				...
				return console.log("Master keys updated.")
			}
...
const decryptCryptoJSFolderName = (str, userMasterKeys) => {
	let folderName = "Cannot decrypt (rename folder to fix)"
	...
	if(folderName == "Cannot decrypt (rename folder to fix)"){
		return doLogout()
	}
	...
	/*
	try{
		let folderEncName = str
		let folderName = ""
		let folderNameFound = false
		let folderNames = []
		...
		userMasterKeys.forEach((key) => {
			if(!folderNameFound){
				try{
					let decFolderName = CryptoJS.AES.decrypt(folderEncName, key).toString(CryptoJS.enc.Utf8)

					try{
						var obj = JSON.parse(decFolderName)

						folderNameFound = true
						folderName = obj.name
					}
					catch(e){
						if(decFolderName.length > 0){
							folderNames.push(decFolderName)
						}
					}
		...
		if(!folderNameFound){
			...
			if(potentialNames.length > 0){
				folderName = potentialNames[potentialNames.length - 1]
			}
			else{
				folderName = folderNames[folderNames.length - 1] || "Cannot decrypt"
			}
		...
		if(typeof folderName == "undefined"){
			if(decryptCryptoJSFolderNameTries >= 32){
				return "Cannot decrypt"
			}
			else{
				decryptCryptoJSFolderNameTries += 1

				folderName = decryptCryptoJSFolderName(str, userMasterKeys.reverse())
			}
		}

		decryptCryptoJSFolderNameTries = 0

		return (folderName.length >= 1 ? folderName : "Cannot decrypt")
	}
	catch(e){
		console.log(e)
...
const downloadFileToLocal = async (path, file, isSync, callback) => {
	...
	if(!fileDirPathExists){
		return callback(new Error("file parent dir does not exist locally"))
	}
										...
										if(thisIndex >= fileChunks){
											...
											if(!markedAsDone){
												...
												markUploadAsDone(uuid, uploadKey, 0, 32, (err) => {
													...
													checkIfItemParentIsBeingShared(parent, parent, "file", {
														uuid: uuid,
														name: name,
														size: parseInt(size),
														mime: mime,
...
const getLocalSyncDirContents = async (callback) => {
	...
	//files = sortObjectArrayByPropLengthASC(files)
	//folders = sortObjectArrayByPropLengthASC(folders)
			...
			list.forEach((file) => {
				...
				fs.stat(file, (err, stat) => {
					if(stat && stat.isDirectory()){
						...
						if(stat){
							modTimes[file] = stat.mtimeMs || 0
							thisFileSize = stat.size || 0
						}
						else{
							modTimes[file] = 0
						}
	...
	walk(userSyncDir, (err, data) => {
		if(err){
			return callback(err)
		}

		if(typeof lastLocalSyncDataHash !== "undefined"){
			if(hashFnFast(JSON.stringify(data)) == lastLocalSyncDataHash){
				return callback(null, lastLocalSyncFolders, lastLocalSyncFiles)
			}
		}
		...
		for(let i = 0; i < data.length; i++){
			let filePath = data[i].substring(userHomePath.length + 1).split("\\").join("/")
			let lastChar = filePath.slice(-1)
			let filePathEx = filePath.split("/")

			if(lastChar == "/"){
				let folderName = filePathEx[filePathEx.length - 2]
				...
				let fileName = filePathEx[filePathEx.length - 1]

				files[filePath] = {
					name: fileName,
					modTime: modTimes[data[i]]
				}
			}
		}
		...
		lastLocalSyncDataHash = hashFnFast(JSON.stringify(data))
...
const getRemoteSyncDirContents = async (folderUUID, callback) => {
	...
	apiRequest("/v1/get/dir", {
		...
		for(let i = 0; i < res.data.folders.length; i++){
			let self = res.data.folders[i]
			let selfName = decryptCryptoJSFolderName(self.name, userMasterKeys)
		...
		for(let i = 0; i < res.data.files.length; i++){
			...
			if(pathsForFiles[self.parent] !== "undefined"){
				let metadata = decryptFileMetadata(self.metadata, userMasterKeys)
				let newPath = pathsForFiles[self.parent] + metadata.name
		...
		//folderPaths = sortObjectArrayByPropLengthASC(folderPaths)
		//files = sortObjectArrayByPropLengthASC(files)
...
const syncTask = async (where, task, taskInfo, userMasterKeys) => {
	...
	if(currentSyncTasks.includes(taskId)){
		...
		/*if(typeof syncTaskTries[taskId] == "undefined"){
			syncTaskTries[taskId] = 0
			...
			if(syncTaskTries[taskId] >= 1024){
				return false
			}
			else{
				syncTaskTries[taskId] = syncTaskTries[taskId] + 1
				...
				case "mkdir":
					apiRequest("/v1/dir/exists", {
						...
						apiRequest("/v1/dir/sub/create", {
							...
							checkIfItemParentIsBeingShared(taskInfo.parent, taskInfo.parent, "folder", {
								uuid: newFolderUUID,
								name: taskInfo.name
							})
						...
						if(!res.status){
							return setTimeout(() => {
								...
								if(res.message.indexOf("already") !== -1){
									localFileModifications[taskInfo.filePath] = undefined
									remoteFileSizes[taskInfo.filePath] = undefined
									remoteFileUUIDs[taskInfo.filePath] = undefined
									localFileExisted[taskInfo.filePath] = undefined
								}
				...
				case "rmdir":
					apiRequest("/v1/dir/trash", {
						...
						return setTimeout(() => {
							...
							localFolderExisted[taskInfo.path] = undefined
		...
		case "local":
			switch(task){
				case "mkdir":
					...
					fs.mkdir(winOrUnixFilePath(dirPath), (err) => {
						addFinishedSyncTaskToStorage(where, task, JSON.stringify(taskInfo))
	...
	getRemoteSyncDirContents(folderUUID, (err, remoteFolders, remoteFiles) => {
		...
		getLocalSyncDirContents(async (err, localFolders, localFiles) => {
							...
							if(fileStat){
								if(typeof localFileModifications[filePath] == "undefined"){
									if(fileStat.mtimeMs > 1399784066403){
										...
										localFileModifications[filePath] = undefined
									}
								}
								else{
			...
			let waitForQueueToFinishInterval = setInterval(async () => {
				if(currentSyncTasks.length < 99999999990){
