...
const removeFoldersAndFilesFromExistingDir = (path, callback) => {
	...
	for(let prop in localFolderExisted){
		...
		prop = userHomePath + "/" + prop

		if(prop.indexOf(path) !== -1){
			delete localFolderExisted[oldProp]

			console.log("DEL FOLDER", oldProp)
		}	
	}
	...
	for(let prop in localFileExisted){
		...
		if(prop.indexOf(path) !== -1){
			delete localFileExisted[oldProp]
			delete localFileModifications[oldProp]
			delete remoteFileSizes[oldProp]
			delete remoteFileUUIDs[oldProp]

			console.log("DEL FILE", oldProp)
		}	
	}
			...
			switch(task){
				...
				case "update":
					...
					downloadFileToLocal(winOrUnixFilePath(filePath), taskInfo.file, true, async (err) => {
						...
						let fileDirEx = taskInfo.filePath.split("/")
