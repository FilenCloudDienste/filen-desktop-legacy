...
const downloadFileToLocal = async (path, file, isSync, callback) => {
	...
	try{
		...
		/*try{
			let res = await fs.mkdir(winOrUnixFilePath(fileDirPath), {
				recursive: true,
				overwrite: true
			})
...
const getRemoteSyncDirContents = async (folderUUID, callback) => {
	...
	apiRequest("/v1/get/dir/cached", {
		...
		//if(res.data.cached){
		//	return callback("Data is cached, skipping.")
		//}
			...
			switch(task){
				...
				case "update":
					...
					downloadFileToLocal(winOrUnixFilePath(filePath), taskInfo.file, true, async (err) => {
						...
						//let fileDirEx = taskInfo.path.split("/")

						//fileDirEx.pop()

						//let fileDirPath = fileDirEx.join("/") + "/"

						//console.log("MAKR DIR AS EXISTING", fileDirPath)

						//localFolderExisted[fileDirPath] = true
				...
				for await (let file of klaw(userSyncDir, {
					...
					preserveSymlinks: true
				  	...
				  	if(file.stats){
				  		...
				  		if(typeof localDataChangedFiles[filePath] !== "undefined"){
				  			if(file.stats.mtimeMs > localDataChangedFiles[filePath]){
				  				localDataChangedFiles[filePath] = file.stats.mtimeMs
